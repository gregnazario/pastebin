...
export class FileTypeTests {
  ...
  static async runAll(): Promise<void> {
    ...
    for (const fileType of FileTypeTests.fileTypes) {
      console.log(`Testing ${fileType.name}...`);

      try {
        const passed = await FileTypeTests.testFileType(fileType, password);
   ...
   */
  private static async testFileType(fileType: FileTypeTest, password: string): Promise<boolean> {
    // Generate test content
    const originalData = fileType.generateContent();
    // Create a new ArrayBuffer-backed Uint8Array for File constructor compatibility
    const arrayBuffer = originalData.buffer.slice(
      originalData.byteOffset,
      originalData.byteOffset + originalData.byteLength,
    ) as ArrayBuffer;
    const file = new File([new Uint8Array(arrayBuffer)], 'test-file', { type: fileType.mimeType });
    ...
    try {
      // Upload (encrypt)
      const uploadResult = await FileTypeTests.fileEncryptionService.uploadFile(
        file,
        password,
        false,
      );
      ...
      // Download (decrypt)
      const downloadResult = await FileTypeTests.fileEncryptionService.downloadFile(
        uploadResult.fileId,
        password,
        privateKeyFragment,
      );
      ...
      if (downloadResult.metadata.mimeType !== fileType.mimeType) {
        throw new Error(
          `Mime type mismatch: expected ${fileType.mimeType}, got ${downloadResult.metadata.mimeType}`,
        );
      }
  ...
  static async testFileSizes(): Promise<void> {
    ...
    for (const { size, label } of sizes) {
      ...
      try {
        ...
        // Create file for reference (used in testFileType via generateContent)
        void new File([data], `test-${label}.bin`, {
          type: 'application/octet-stream',
        });

        // Test upload/download
        await FileTypeTests.testFileType(
          {
            name: label,
            mimeType: 'application/octet-stream',
            generateContent: () => data,
            validate: (a, b) => FileTypeTests.arraysEqual(a, b),
          },
          password,
        );
