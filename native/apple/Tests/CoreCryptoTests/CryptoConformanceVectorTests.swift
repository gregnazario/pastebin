...
/// Conformance test proving Swift native engine can decrypt a web-generated vector.
struct CryptoConformanceVectorTests {
    @Test func productionEngineDecryptsWebConformanceVector() async throws {
        let vector = try JSONDecoder().decode(Vector.self, from: Data(vectorJSON.utf8))
        let engine = ProductionNativeCryptoEngine()

        let serializedPayload = try #require(decodeBase64URL(vector.serializedPayloadBase64Url))
        let plaintext = try #require(decodeBase64URL(vector.plaintextBase64Url))

        let result = try await engine.decrypt(
            serializedPayload: serializedPayload,
            password: vector.password,
            privateKeyBase64Url: vector.privateKeyBase64Url
        )

        #expect(result.plaintext == plaintext)
        #expect(result.metadata == vector.metadata)
    }
